...
class UserMetricsSerializer(serializers.Serializer):
    ...
    sessions_per_week_ratio = serializers.FloatField(default=0)
    session_count_current_week = serializers.IntegerField(default=0)
    session_count_last_week = serializers.IntegerField(default=0)
    session_minutes_current_week = serializers.FloatField(default=0)
    session_minutes_last_week = serializers.FloatField(default=0)
    total_exercise_logs = serializers.IntegerField(default=0)
    logs_current_week = serializers.IntegerField(default=0)
    logs_last_week = serializers.IntegerField(default=0)
    total_volume_current_week = serializers.FloatField(default=0)
    total_volume_last_week = serializers.FloatField(default=0)
    logs_per_week_ratio = serializers.FloatField(default=0)
...
class ExerciseSerializer(serializers.ModelSerializer):
    workouts = serializers.PrimaryKeyRelatedField(
        many=True, queryset=Workout.objects.all(), write_only=True, required=False
    )
    ...
    def update(self, instance: Exercise, validated_data):
        ...
        workouts_updated = Workout.objects.filter(
            ...
            + [x.id for x in workout_instances]
        )
        instance.name = validated_data.get("name", instance.name)
        instance.description = validated_data.get("description", instance.description)
...
class ExerciseLogSerializer(serializers.ModelSerializer):
    session = serializers.PrimaryKeyRelatedField(queryset=WorkoutSession.objects.all())
...
class WorkoutSessionSerializer(serializers.ModelSerializer):
    workout = serializers.PrimaryKeyRelatedField(
        queryset=Workout.objects.all(), required=True
    )
